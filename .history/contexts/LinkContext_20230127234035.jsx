...
const LinkContextProvider = ({ children }) => {
    ...
    useEffect(  () => {
       const fetchLink =()=>
          {  
            DataStore.query(Link).then((links) =>
            setLinks(links)
           );
          }
           fetchLink();
...
/* Fetch Image */
 ...
 useEffect(()=>{
  const fetchImage=async()=>{
   links.forEach((link)=>{const params = {
      Bucket: 'portfolioml26151fd83d4a40cb89e358a0b8c234d582358-staging',
      Key: link?.image,
    };
    s3.getSignedUrlPromise('getObject', params).then((i)=>setImage(i));})
  }
 fetchImage();
 },[dbUser])
 ...
 */
  ...
  return (
    <LinkContext.Provider value={{ links,image }}>
      {children}
